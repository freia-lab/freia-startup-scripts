...
export LD_LIBRARY_PATH=/epics/base-7.0.7/extensions/pcas-e075fd450/lib/linux-x86_64:/epics/base-7.0.7/lib/linux-x86_64
...
#	EPICS_CA_MAX_ARRAY_BYTES=20000000 gateway -home $gwhome -log gateway.log -prefix GW-$host -cip 192.168.10.255 -sip 130.238.200.140 -signore $host_ip -archive -no_cache -pvlist /opt/startup/boot/$host/gateway/GATEWAY.pvlist -access /opt/startup/boot/$host/gateway/GATEWAY.access -uid $uid -gid $gid -server
	EPICS_CA_MAX_ARRAY_BYTES=20000000 gateway -home $gwhome -log gateway.log -prefix GW-$host -cip 192.168.10.255 -sip 130.238.200.140 -signore $host_ip -archive -no_cache -pvlist /opt/startup/boot/$host/gateway/GATEWAY.pvlist -access /opt/startup/boot/$host/gateway/GATEWAY.access -server
}
